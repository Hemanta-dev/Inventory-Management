  ...
  const deleteItemsRequest = async (id) => {
    try {
      ...
      if (res.status === 200) {
        ...
        const updatedItemsRes = await fetch('/items-list', {
          headers: {
            Accept: 'application/json',
            Authorization: 'Bearer ' + cookieValue,
          },
        ...
        if (updatedItemsRes.status === 200) {
          const updatedItems = await updatedItemsRes.json();
          console.log(updatedItems,"99999999999999999999999" )
          setDeleteItems(updatedItems.itemsList);
...
/******************when i clicked in Request user Item list start***************************/
...
/*********************when i clicked in Request user Item list start ends********************/
  ...
  const tabs = [
    ...
    {
      id: 'example6',
      name: <div onClick={() => { setselectedTab(tabs[5]); }}>Logout</div>,
      content: (
